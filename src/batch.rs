...
impl<E: MultiMillerLoop> BatchInfo<E> {
    pub fn build_aggregate_circuit(&self, cache_folder: &Path) -> CircuitInfo::<E, AggregatorCircuit<E::G1Affine>> {
        // 1. setup params
        let params =
            load_or_build_unsafe_params::<E>(self.k, &cache_folder.join(format!("K{}.params", self.k)));
