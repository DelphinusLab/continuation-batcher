...
RUST_BACKTRACE=1 CUDA_VISIBLE_DEVICES=1 cargo run --features perf --features profile --release -- --params $WKDIR/params --output $WKDIR/output batch -k 23  -s shplonk --challenge keccak --accumulator use-hash --info $WKDIR/output/fibonacci.loadinfo.json --name fib_agg --commits $BATCH_INFO_INIT $BATCH_INFO_RECT $BATCH_INFO_FINAL --cont 6 \
    2>&1 | rotatelogs -e -n 10 ./batchTest.log 100M
